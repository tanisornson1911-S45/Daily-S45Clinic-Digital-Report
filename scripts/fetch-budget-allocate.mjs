...
// "Total" ไม่มีทางเจอในลูปด้านล่างอยู่แล้ว (ลูปหยุดก่อนถึง totalRowIdx) — ใส่กันเหนียวไว้เฉยๆ
// "Inter" เคยถูกข้ามตรงนี้เพราะมีโครงสร้างซ้อน 3 ชั้น (Inter > หัตถการย่อยที่มาจาก Inter เช่น "Nose Open"/
// "ยกคิ้ว/เลื่อนไรผม"/"หน้าอก" > คุณหมอ) ต่างจากหัตถการอื่นที่เป็น 2 ชั้นตรงๆ — แต่การข้ามแบบนั้นมีบั๊กจริง:
// พอ "Inter" ไม่ถูกนับเป็นขอบเขตหัตถการ แถวทั้งบล็อกของ Inter (รวมถึงแถวคุณหมอย่อยของมัน) จะถูกดูดเข้าไป
// เป็น "แถวย่อย" ของหัตถการก่อนหน้า (เช่น "เสริมจมูก Semi Open") แทน แล้วชื่อคุณหมอที่ซ้ำกัน (หมอโรส/หมอเช
// ปรากฏทั้งใน Semi Open เองและใน Inter) จะเขียนทับค่าจริงของ Semi Open ด้วยค่าที่มาจาก Inter แบบเงียบๆ —
// ตรวจพบจริงจากการ diagnostic-dump แถว 37-50 ของชีต September26 เมื่อ 2569-09-14 — แก้โดยนับ "Inter" เป็น
// ขอบเขตหัตถการตามปกติ (กัน Semi Open ไม่ให้โดนดูดข้อมูลของ Inter ปนเข้าไป) แล้วดึงงบรายคุณหมอของ Inter เอง
// ด้วย parser ทั่วไปเดียวกัน — แถวหัตถการย่อยของ Inter ("Nose Open"/"ยกคิ้ว/เลื่อนไรผม"/"หน้าอก") ไม่มีคำว่า
// "หมอ" จึงถูกกรองทิ้งโดย looksLikeDoctorName() อัตโนมัติ เหลือแต่แถวคุณหมอจริงที่ซ้อนอยู่ข้างใน
const SKIP_TOP_LEVEL_LABELS = new Set(["Total"]);

// ดึงงบแยกรายคุณหมอของ "ทุกหัตถการ" (รวม Inter) จากชีต Budget Allocate — ใช้เป็นฐานข้อมูลของแผนปรับ
...
// คุณหมอ (Target ว่าง) จะอยู่ถัดไปจนกว่าจะเจอแถวหัตถการบนสุดถัดไปหรือแถว "Total". คุณหมอคนเดียวกันอาจ
// ปรากฏซ้ำหลายแถวภายใต้หัตถการเดียวกัน (โดยเฉพาะ Inter ที่ซ้อน 3 ชั้น คุณหมอคนเดียวรับเคสจากหลาย
// หัตถการย่อยของ Inter ได้) — รวมยอด (บวกสะสม) แทนการเขียนทับ เพื่อไม่ให้งบที่แท้จริงหายไป
function parseAllProcedureDoctorBudgets(sheet, title) {
  ...
  for (let k = 0; k < topLevelIdxs.length; k++) {
    ...
    for (let i = rowIdx + 1; i < subRowEnd; i++) {
      ...
      const existing = doctorsByName[name];
      if (existing) {
        existing.budgetSet += budgetSet;
        existing.currentSpend += num(subRow[currentSpendCol]);
        existing.targetChat += num(subRow[targetChatCol]);
        existing.actualChat += num(subRow[actualChatCol]);
        continue;
      }
      doctorsByName[name] = {
        name,
      ...
      JSON.stringify(
        {
          generatedAt: new Date().toISOString(),
          source:
            "Generated by scripts/fetch-budget-allocate.mjs — งบแยกรายคุณหมอของทุกหัตถการ (รวม Inter) " +
            `ของเดือนล่าสุด (${procedureBudget.tabTitle}) จากชีต "S45 - Budget Allocate". ใช้เป็นฐานข้อมูลของ ` +
            "\"แผนปรับงบประมาณ\" บนหน้า Ads — แถวคุณหมอที่งบเป็น 0 เดือนนี้ (เช่นหมอตี้ในเสริมจมูกโอเพ่น) และแถวไม่ใช่ " +
            "คุณหมอ (Awareness ฯลฯ) ไม่รวมอยู่ในแผนนี้. Inter มีโครงสร้างซ้อน 3 ชั้น (Inter > หัตถการย่อยที่มาจาก " +
            "Inter > คุณหมอ) — คุณหมอที่รับเคสจากหลายหัตถการย่อยของ Inter จะถูกรวมยอดงบ/แชทเป็นแถวเดียว. " +
